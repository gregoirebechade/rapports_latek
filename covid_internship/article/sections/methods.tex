...
As the goal of this study is to compare the accuracy of forecast in the context of many different pandemics, a large set of synthetic pandemics was generated, with a particular attention on the diversity of these pandemics. 
...
This model takes as an input many parameters such as the population type, the population size, the age distribution etc, and outputs a complete description of the pandemic, with real-time values of each relevant piece of information, such as the number of severe, of asymptomatic etc, but also physical data such as the value of the reproduction number. 
...
\subsubsection{First pandemic}

For the implementation and the first tests of the models, a single pandemic was generated. 

\input{tables/table_covasim_parameters.tex}


The parameters used to generate this pandemic are described in the table \ref{tab:parameters_covasim} 
The parameters that are not specified are the default parameters of the Covasim library. 



The interventions (interventions1) were based on mobility reports from Västtrafik (Fig.\ref{fig:mobility_reports}), the public transport company of the city of Gothenburg. 
They were reported during the Covid-19 pandemic and have been retrieved from \cite{gerlee2021predicting}.
...
As the interventions refer to relative changes in mobility, they were used as relatives changes in probabilities of transmission. 
\begin{figure}[h]
    \centering
...
A huge diversity of pandemics is needed to determine the most accurate forecasting model.
It is therefore relevant to identify the key parameters that generate this diversity.
As Covasim has a large set of inputs parameters, a first subset of key parameters was identified: the \textit{spread} parameters and the \textit{severity} parameters, which are parameters related to disease transmission and severity
The \textit{severity} parameters are the 4 parameters that correspond to the probability for an individual to move from one disease compartment to another. 
The \textit{spread} parameters are 9 parameters that represent the distribution of probability of the time spend by an agent in a compartment (such as infected, crictical...) once he entered it. 
This distribution is a log-normal distribution, but the \textit{spread} parameters correspond to the mean of this log-normal distribution
...
In order to test the forecasting models on a large, but still manageable set of different pandemics we decided to vary four parameters and that these should each take three different values in $[0.5, 1, 2]$, leading to a set of 81 pandemics. 
To select the 4 parameters that generated the most diversity out of the 13 possible, different diversity metrics were constructed. 
...
\begin{aligned}
    ...
    &\quad \quad\quad\quad\quad \quad\quad\quad \quad\quad \quad\quad\quad\quad \quad\quad\quad\quad \quad\quad\text{ with }Y' \text{ and } Y'' \text{the first and second numerical derivatives of } Y\\
    ...
    &\quad \quad\quad\quad\quad \quad\quad \quad\quad \quad\quad\quad\quad \quad\quad\quad\quad \quad\quad\quad\text{ with }Y' \text{ and } Y'' \text{the first and second numerical derivatives of } Y\\[1cm]
\end{aligned}
$

It can be noted that $\mathcal{L}_2$ is similar to the Sobolev norm  $\| \tilde{Y_1}  - \tilde{Y_2} \|_{W^{2, 1}} $ with squared terms and with additionary terms taking into account the amplitude. 

To determine which measure to use, we generated 14 pandemics : one reference pandemic with all parameter values equal to the default values and the remaining pandemics having one parameter value doubled.
...
But, keeping the parameters from this evaluation (\texttt{[0, 5, 10, 12]}) would not be accurate, as the parameters were changed independently, and the diversity did not take into account the correlation between some of them. \\

Finding the parameters that maximise the $\mathcal{L}_2$ diversity is equivalent to solving the following problem : \\
...
An MCMC algorithm \cite*{diaconis2009markov} was therefore implemented, to perform a clever grid search on the different subsets $s \subset S $ of parameters.
The MCMC algorithm is a method which is used to sample from a distribution that cannot be directly sampled. 
...
This algorithm enables to perform a clever grid search, as the Markov Chain tends to increase the diversity but is authorized to sometimes diminishes it to avoid to stay stucked in a local maximum, as a mere gradient descent would do. 
...
To prove that the stationary distribution of this Markov chain is $\pi$, we will show that it is an irreducible and aperiodic Markov chain and that $\pi$ is reversible for $K$. \\
...
After 200 iterations, the set of parameters that maximised the diversity was found to be \texttt{[2, 4, 9, 10] }, which correspond to the parameters \texttt{[sym2sev, asym2rec, rel\_symp\_prob, rel\_severe\_prob]}. 
...
To create the most diverse set possible, we also created 4 different time-dependent mobilities (Fig.\ref{fig:mobilities}), corresponding to constant mobility, annual variations, lockdown scenario and the mobility data from Västraffik from \cite{gerlee2021predicting}. 
\begin{figure}
    \centering
...
Indeed each of the 4 parameters was set to three different values : \texttt{[0.5, 1, 2]} and the 4 time-dependant mobilities were used.


\subsection{The models}

In this study, we define a model ${h}_{\theta }$ as a function ${h}$  defined on $ \mathbb{N}$ (corresponding to the days since the start of the pandemic ), with parameters $\theta$ and trained on the data $\mathcal{D}$.
In the training phase, $\hat{\theta}$,  an estimator of $\theta$ is computed from $\mathcal{D}$, and used for the prediction.  
We considered two types of models: the first type correspond to models which are only trained on the time series we want it to predict (the number of hospitalized in our case), and the second type of models are trained on the time series we want to predict, but also on other time series that can be relevant to predict the number of hospitalized (the mobility and the number of infected). 
...
During the training or predicting phase, the computation sometimes fail (for instance, when the matrix is non-invertible for the linear regression model). 
Instead of outputing a missing value, we let the model output the value of the moving average model (see \ref{sec:moving_average}), which can be interpreted as a naive output when the computation fails. \\[0.3cm]
...
The model was trained on the data $\mathcal{D}$ to compute $\hat{\theta}$ the parameter estimator and the output $\hat{Y}_r = {h}_{\hat{\theta}}(r)$.
...
Each compartment corresponds to the number of individuals in the health state corresponding to the compartment. 
The evolution of the number of individuals in each compartment is described by a system of ordinary differential equations: 
...
As the system of equation cannot be directly solved, we use a Euler method to solve it : \\


We chose to fix $dt=0.001$ in the Euler's method. 
...
We implemented some variations of the model in which either $\gamma_i$, $\gamma_h$ or both were fixed to the value $0.2$ (see the Table.\ref{tab:sirh_models_parameters}).
...
It has the same structure but uses the mobility data and the number of infected in order to provide more accurate forecasts. 
...
The normalization factors enables to put equal weight on the number of infected and hospitalized. 
...
We performed a grid search on a single pandemic to identify the combination of parameters that would optimize the prediction accuracy. 
...
Again, the $\phi_{i,j,k}$ and  $c_i$ are estimated through maximum likelihood estimation with the \texttt{statsmodel} library. 
...
It returns a constant prediction that correspond to the mean of the 7 past days, whatever the reach $r$. 
...
We then trained and optimized both regressors : the linear and the bayesain regressors, which were the only ones that provided reasonable outputs on the \texttt{scikit-learn} models including : linear regression, bayesian regression, Gradient boosting regressor, Random Forest regressor and support vector regression. 
The Bayesian ridge model was implemented with the default prior of the \texttt{scikit-learn} library (a gamma distribution with shape parameter equal to $10^{-6}$ and scale parameter equal to $10^{-6}$).
...
If we have new data $\tilde{X} \in \mathbb{R}^{1 \times d}$ that we want to predict, the prediction is given by : \\
...
$
\newline
It implies that $\tilde{Y}$ follows a normal distribution of expected value $\tilde{X} \beta$ and variance $\tilde{X} (X^T X)^{-1} \tilde{X}^T \sigma ^2$.\\
...
It has been showned (see \cite{cramer2022evaluation} and \cite{reich2019accuracy}), that ensemble models, which combine the outputs of many models, can outperform individual models.
...
Indeed, the exponential models almost always output very high values, and it had as a consequence that the optimization step of the linear regression problem failed. 
...
As the ensemble model only outputs a single value without confidence intervals, it is only evaluated with the RMSE metric (see \ref{sec:metrics}). 
\begin{figure}
    \centering
...
\end{equation}
This assumption is a condition for the sequence of the gradient of $h_\theta$ not to move too much as $n$ goes to infinity. 
This is a strong assumption, but required to have some essential convergences for the proof. 
Let:
\begin{align*}
Y &= \begin{pmatrix}
Y_1 \\
...
There exists a law of large number for independent and non identically distributed random variables, which asserts that (see \cite{suquet2004lois}):
...
Finally, with Slutsky (\cite{van2000asymptotic}), we obtain that: 
...
$\nabla_\theta h_\theta [i] \simeq \frac{h_{\theta + d \theta _ i  } - h_\theta}{dt}$, with $dt=0.0001$ and  \\
...
$IS_\alpha( [l, u],  y ) = \frac{2}{\alpha} \times ( \mathbb{1}_{\{y<l\}} (l-y) + \mathbb{1}_{\{y>u\}} (y-u) + (u-l))$. \\
...
$WIS( [l, u], y ) = \sum_{k=0}^{K} w_k IS_{\alpha_k}( [l, u], y ) $ , with $(w_k)_{k \in \{1, ... , K\}} \in \mathbb{R}_+ ^K $ weights chosen by the user. \\
...
For the analysis of the results, we decided to remove the points for which the number of hospitalized was below 100, (i.e less than $10^{e-4}$ hospitalized per 100 000 citizens) both point classification and model predictions were irrelevant at this period of the pandemic.
Moreover, it is pointless to assess the performances of the models during the period in which there is no pandemic, as the interesting models are the ones that can predict the number of hospitalized during the waves.
...
In order to compare the performance of the models at different point of the pandemic, we classified each day of the pandemic in six categories : \textit{stable, increase, big increase, decrease, big decrease}.
The $R_{eff}$ number was used for point classification. 
...
For a pandemic $X_{i, i \in [1, ..., n]}$ of reproduction number $R_{i, i \in [1, ..., n]}$, and a day $d \in [1, \hdots, n]$, the classification is made according rule in \ref{tab:classification}. 




\input{tables/table_classification.tex}

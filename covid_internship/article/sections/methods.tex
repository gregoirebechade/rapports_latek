...
The $\mathcal{L}_2$- diversity increased from 62 353 to 93 553. \\
...
Instead of outputting a missing value, we let the model output the value of the moving average model (see \ref{sec:moving_average}), which can be interpreted as a naive output when the computation fails. \\[0.3cm]
...
For the analysis of the results, we decided to remove the points for which the number of hospitalized was below 100, (i.e less than $10^{-4}$ hospitalized per 100 000 citizens) both point classification and model predictions were irrelevant at this period of the pandemic.
...
For a pandemic $X_{i, i \in [1, ..., n]}$ of reproduction number $R_{i, i \in [1, ..., n]}$, and a day $d \in [1, \hdots, n]$, the classification is made according Table.\ref{tab:classification}. 

...
It has been showned ( \cite{cramer2022evaluation} and \cite{reich2019accuracy} ) that ensemble models, which combine the outputs of many models, can outperform by far individual models.
...
\]
We also suppose that $h_\theta$ is differentiable and that  $ \exists K \in \mathbb{R},  \forall i \in \mathbb{N}, \left\lVert  \nabla_\theta h_{\theta^*}(i) \right\rVert^2 \leq  K$.
Let:
\begin{align*}
Y &= \begin{pmatrix}
Y_1 \\
...
Let $n \in \mathbb{N}^*$. Let $i \in \{1, ..., n\}$.
Let $(a_{i, j})_{j \in \{1, ..., d\}}$ be the values of $A_i$.
As $\left\lVert A_i \right\rVert ^2 \leq K$, we have that $ \forall j \in \{1, ..., d\}  , |a_{i, j}|^2 \leq K$
$\sum_{i=0}^{n} $
...
% On utilise Th´eor`eme 12  du papier LFGN dans les téléchatgements pour avoir la cvgence de d^ et D^ qui sont des va indépendantes mais pas identiquement dsitribuées. 
\[

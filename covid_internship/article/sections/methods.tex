...
Let $(A_i, \tilde{Y_i})$ be the observations. ATTENTION LES EPSILON I SONT DES EPSILON I ' , IL FAUT AUSSI CHANGER LES SIGMA 
We suppose that the $A_i$ are independant and identically distributed. 
Let $\mathbb{P}$ be the law from which the $A_i$ are drawn, and let us assume that $Y_i = A_i \beta ^* + \epsilon_i $, with $\epsilon_i \sim \mathcal{N}(0, \sigma^2)$.
IL MANQUE DES TILDE ?
...
\] (A EST CONSTANT , IL FAUT CHANGER CETTE PARTIE. Si $\nabla_\theta h_\theta$ est borné, on a que $\hat{D}$ converge )

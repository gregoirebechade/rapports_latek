...
The first value of the sequence was a clever starting point: the four parameters that maximized $\mathcal{L}_2$ on the 14 pandemics generated (Fig.\ref{fig:diversity_pandemics}).  
...
We implemented some variations of the model in which $\gamma_i$, $\gamma_h$ or both were fixed to the value $0.2$ (see the Table.\ref{tab:types_of_sirh}).
...
It has been showned ( \cite{cramer2022evaluation}a dn \cite{reich2019accuracy} ) that ensemble models, which combine the outputs of many models, can outperform by far individual models.
